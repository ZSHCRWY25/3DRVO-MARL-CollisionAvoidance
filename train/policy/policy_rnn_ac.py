...
class rnn_ac(nn.Module):
    ...
    def __init__(self, observation_space, action_space, state_dim, rnn_input_dim=9, 
    rnn_hidden_dim=64, hidden_sizes_ac=(256, 256), hidden_sizes_v=(16, 16), 
    activation=nn.ReLU, output_activation=nn.Tanh, output_activation_v= nn.Identity, use_gpu=True, rnn_mode='GRU', drop_p=0):
        super().__init__()
...
class GaussianActor(Actor):
    ...
    def _distribution(self, obs, std_factor=1):
        if isinstance(obs, list):
            ...
            net_out = self.net_out(obs)#获取动作的均值 mu
        else:
        ...
        if torch.isnan(obs).any() or torch.isinf(obs).any():
            print(obs)
            raise ValueError("观测数据包含 NaN/Inf！")

        
        mu = net_out 
        std = torch.exp(self.log_std)
        std = std_factor * std + 1e-6 
        std = torch.clamp(std, min=1e-4, max=10.0)

        if torch.isnan(mu).any() or torch.isinf(mu).any():
            print(f"异常 mu 值: {mu}")
        if torch.isnan(std).any() or torch.isinf(std).any():
            print(f"异常 std 值: {std}")
